...
if (!/^((idea|news|tool):\s*\S|\/?(idea|retrysite|post)$)/i.test(text)) {
  console.error('usage: npm run trigger -- "idea:|news:|tool: <what it is>"   |   npm run trigger -- idea|retrysite|post');
  process.exit(1);
}
